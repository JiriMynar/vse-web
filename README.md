...
- `DATA_DIR` – cesta k adresáři, kam se ukládá SQLite databáze (v produkci důrazně doporučujeme nastavit na trvalý disk, např. `/var/data` na Renderu; pokud chybí **nebo není zapisovatelná**, aplikace vypíše varování, přepne se na nejbližší dostupný adresář a aktualizuje si proměnnou prostředí `DATA_DIR`, aby všechny části aplikace pracovaly s fallbackem)
...
- Databáze SQLite se v lokálním vývoji ukládá do adresáře `data/` (případně do `/tmp/vse-web/data`), ale v produkci by měla směřovat na trvalé úložiště. Pokud proměnná `DATA_DIR` chybí, aplikace se pokusí použít běžné mount pointy (např. `/var/data`), zároveň ale v logu upozorní, že proměnnou je vhodné nastavit explicitně.
